  ...
  config = mkIf cfg.enable (
    let
        ...
        if trackCfg.package != null then trackCfg.package
          ...
          let
            bundle = 
              ...
              else pkgs.mkCuttlefishFromDeb { debs = trackCfg.debs; };
          in pkgs.mkCuttlefishFHS { 
      ...
      baseBundle =
        ...
        else pkgs.mkCuttlefishFromDeb { debs = cfg.debs; };

      fhsPackage =
        if cfg.package != null then cfg.package
        else pkgs.mkCuttlefishFHS { cuttlefishBundle = baseBundle; };
