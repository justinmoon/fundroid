...
fn run() -> Result<()> {
    ...
    info!("opening KMS device {card_path}");
    let file = OpenOptions::new()
        .read(true)
        .write(true)
        .open(card_path)
        .with_context(|| format!("failed to open {card_path}"))?;
    let mut card = Card(file);
    ...
    fill_buffer_with_color(&mut card, &mut dumb, (255, 136, 0))
    ...
    info!("framebuffer active; sleeping to keep display visible");
...
fn find_connected_connector(
    ...
    for handle in res_handles.connectors() {
        ...
        info!("connector {handle:?} state {state:?}");
