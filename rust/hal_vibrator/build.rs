...
fn main() {
    ...
    if target_os != "android" {
        println!(
            "cargo:warning=hal_vibrator targets Android; skipping shim linkage for {}",
            target_os
        );
        return;
    }
    ...
    if !lib_dir.exists() {
        panic!(
            "vibrator shim archive directory not found: {}",
            lib_dir.display()
        );
    }

    println!("cargo:rustc-link-search=native={}", lib_dir.display());
    println!("cargo:rustc-link-lib=static=vibrator_shim");
